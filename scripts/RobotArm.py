...
J1 = IMU(ref=quaternion_from_euler(0, math.pi/2, 0))
J2 = IMU(ref=quaternion_from_euler(math.pi/2, 0, math.pi/2))
J3 = IMU(ref=quaternion_from_euler(math.pi/2, 0, math.pi/2))
J4 = 0.0
is_running = False
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "config":
            if is_imu_connected:
                ...
                q_imu2 = J2.read_raw()
                q_imu3 = J3.read_raw()
                data = {'imu1': list(q_imu1), 'imu2': list(
                    q_imu2), 'imu3': list(q_imu3)}
